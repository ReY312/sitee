...
-- Основная функция (используется текущим backend API):
-- p_full_name, p_snils, p_visit_date + параметры слотов.
...
-- Legacy-совместимая перегрузка (для старого/переходного backend):
-- p_full_name, p_snils, p_appointment_at.
create or replace function public.create_queue_request(
  p_full_name text,
  p_snils text,
